...
- 連接池優化 (HTTP 客戶端連接池，連接重用指標)
- 記憶體池優化 (sync.Pool 緩衝區、位元組切片、物件池)
...
| ✅ 壓縮狀態顯示 | 新增壓縮啟用狀態、當前壓縮等級顯示 (開發除錯用) | README.md, CLAUDE.md |
...
| 開發測試維護 | 基準測試維護、簡易記憶體檢查、競態條件掃描 | CLAUDE.md |
