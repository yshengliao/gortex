...
func TestGortexRouter_Basic(t *testing.T) {
	router := http.NewGortexRouter()

	// Test basic route registration
	router.GET("/users", func(c http.Context) error {
		return c.String(200, "users")
	})

	router.POST("/users", func(c http.Context) error {
		return c.String(201, "created")
	})

	// Test route with parameter
	router.GET("/users/:id", func(c http.Context) error {
		id := c.Param("id")
...
func TestGortexRouter_Group(t *testing.T) {
	...
	api.GET("/users", func(c http.Context) error {
...
func TestGortexRouter_Middleware(t *testing.T) {
	...
	router.GET("/test", func(c http.Context) error {
