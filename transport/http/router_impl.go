...
func (r *router) Group(prefix string, m ...MiddlewareFunc) Router {
	group := &routeGroup{
		prefix:      prefix,
		middlewares: m,
		router:      r,
...
func (r *router) addRoute(method, path string, h HandlerFunc, m ...MiddlewareFunc) {
	r.mu.Lock()
	defer r.mu.Unlock()
