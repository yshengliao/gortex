...
type MockContext struct {
	req         *http.Request
	res         httpctx.ResponseWriter
	params      map[string]string
	paramNames  []string
	paramValues []string
	store       map[string]interface{}
	path        string
	handler     httpctx.HandlerFunc
...
func NewMockContext(method, path string) *MockContext {
	...
	return &MockContext{
		req:      req,
		res:      httpctx.NewResponseWriter(rec),
...
func (c *MockContext) Build() httpctx.Context {
	return c
}
...
func (c *MockContext) Handler() httpctx.HandlerFunc {
	return c.handler
}

// SetHandler sets the handler
func (c *MockContext) SetHandler(h httpctx.HandlerFunc) {
	c.handler = h
}
...
func (c *MockContext) Reset(r *http.Request, w http.ResponseWriter) {
	c.req = r
	c.res = httpctx.NewResponseWriter(w)
