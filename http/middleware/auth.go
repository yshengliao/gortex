...
func RequireRole(requiredRole string, claimsKey ...string) router.Middleware {
	key := "jwt-claims"
...
func RequireGameID(claimsKey ...string) router.Middleware {
	key := "jwt-claims"
	if len(claimsKey) > 0 && claimsKey[0] != "" {
		key = claimsKey[0]
	}
...
func GetClaims(c router.Context, claimsKey ...string) *auth.Claims {
	key := "jwt-claims"
	if len(claimsKey) > 0 && claimsKey[0] != "" {
		key = claimsKey[0]
	}
...
func GetClaimsFromContext(ctx context.Context, claimsKey ...string) *auth.Claims {
	key := "jwt-claims"
	if len(claimsKey) > 0 && claimsKey[0] != "" {
		key = claimsKey[0]
	}
...
func GetUserID(c router.Context, claimsKey ...string) string {
...
func GetUsername(c router.Context, claimsKey ...string) string {
	if claims := GetClaims(c, claimsKey...); claims != nil {
...
func SessionAuth(store SessionStore) router.Middleware {
...
func SessionAuthWithConfig(config *SessionConfig) router.Middleware {
	// Apply defaults
	if config == nil {
